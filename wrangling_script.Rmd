...
labels <- sprintf(
  ...
  ca_gull$common_name2, ca_gull$gull_value
...
ca <- read_sf(dsn = ".", layer = "california_county_shape_file") # Read data
st_crs(ca) = 4326 # Set CRS
gullmean <- read_csv("seagulls_mean_raw.csv") %>% 
  spread(month, mean) %>% 
  replace(is.na(.), 0) %>% 
  rename('NAME' = 'county') %>% 
...
write.csv(gullmean, "gullmeanca.csv")
...
```{r}
gulls <- read_csv("seagulls_mean_raw.csv")
gull_types <- read_csv("Gull_types.csv")


gull_df <- full_join(gulls, gull_types)
gull_df$gull_type <- as.factor(gull_df$gull_type) 
levels(gull_df$gull_type) <-  c("Large Dark-Backed Gulls", "Gray-Backed White-Headed Gulls", "Small to Medium Sized Gulls", "Rarities")
  

class(gull_df$gull_type)
levels(gull_df$gull_type)
